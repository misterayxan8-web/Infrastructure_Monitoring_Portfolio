...
# Verify Grafana data source in UI
# Configuration → Data Sources → Prometheus → Save & Test
```

#### 3. Exporters Not Running

```bash
# Check service status
sudo systemctl status metrics-exporter service-checker

# View logs
sudo journalctl -u metrics-exporter -f
sudo journalctl -u service-checker -f

# Restart services
sudo systemctl restart metrics-exporter service-checker
```

#### 4. Ports Not Listening

```bash
# Check all ports
sudo ss -tlnp | grep -E '(9090|9100|9101|9102|3000|9200)'
...
```bash
#!/bin/bash
# verification.sh - Run this to verify all components

echo "=== Checking Services ==="
systemctl is-active prometheus && echo "✓ Prometheus OK" || echo "✗ Prometheus FAILED"
systemctl is-active grafana-server && echo "✓ Grafana OK" || echo "✗ Grafana FAILED"
systemctl is-active node_exporter && echo "✓ Node Exporter OK" || echo "✗ Node Exporter FAILED"
systemctl is-active metrics-exporter && echo "✓ Metrics Exporter OK" || echo "✗ Metrics Exporter FAILED"
systemctl is-active service-checker && echo "✓ Service Checker OK" || echo "✗ Service Checker FAILED"

echo ""
echo "=== Checking Endpoints ==="
curl -sf http://localhost:9090/-/ready > /dev/null && echo "✓ Prometheus (9090) OK" || echo "✗ FAILED"
curl -sf http://localhost:9100/metrics > /dev/null && echo "✓ Node Exporter (9100) OK" || echo "✗ FAILED"
curl -sf http://localhost:9101/metrics > /dev/null && echo "✓ Metrics Exporter (9101) OK" || echo "✗ FAILED"
curl -sf http://localhost:9102/metrics > /dev/null && echo "✓ Service Checker (9102) OK" || echo "✗ FAILED"

echo ""
echo "=== Sample Metrics ==="
curl -s http://localhost:9101/metrics | grep -E "achievement_energy|achievement_cost" | head -5
```
